...
public class SchemeBufferInstance extends NpcInstance
{
        ...
        else
        {
        	...
        	if ((int)(System.currentTimeMillis()/1000) > player.getblockUntilTime())
            {
                if (!ENABLE_VIP_BUFFER || ENABLE_VIP_BUFFER && player.getAccessLevel() == VIP_ACCESS_LEVEL)
                {
                    ...
                    {	
                    	msg.setHtml(showText(player, "Info", "Your level is too low!<br>You have to be at least level <font color=LEVEL>" + MIN_LEVEL + "</font>,<br>to use my org.l2j.scripts.services!", false, "Return", "main"));
                    }
            ...
            {
            	msg.setHtml(showText(player, "Sorry", "You have to wait a while!<br>if you wish to use my org.l2j.scripts.services!", false, "Return", "main"));
            }
	...
	private String main(final Player player)
	{
      ...
      /*  if (MESSAGE.length() > 0)
        {
            MAIN_HTML_MESSAGE += "<BR1><table width=100% border=0 cellspacing=0 cellpadding=1 bgcolor=444444><tr>"
                              +  "<td><font color=00FFFF>Preset:</font></td><td align=right><font color=LEVEL>" + org.l2j.scripts.Util.formatAdena(BUFF_SET_PRICE) + "</font> adena</td></tr></table>"
                              +  "<BR1><table cellspacing=0 cellpadding=0>" + MESSAGE + "</table>";
            MESSAGE = "";
            td = 0;
        }

package org.l2j.scripts.npc.model.events;
...
public class LettersManagerInstance extends NpcInstance
{
	...
	public void onMenuSelect(Player player, int ask, long reply)
	{
		if(ask == -1000)
		{
			if(reply == 1)
			{
				for(int[] item : LINEAGE_II_WORD)
				{
					...
					{
						showChatWindow(player, "org/l2j/scripts/events/in_the_pursuit_of_letters/" + getNpcId() + "-no_have_lineageii.htm", false);
						return;
					}
				}
			...
			else if(reply == 2)
			{
				for(int[] item : MEMORIES_WORD)
				{
					if(!ItemFunctions.haveItem(player, item[0], item[1]))
					{
						showChatWindow(player, "org/l2j/scripts/events/in_the_pursuit_of_letters/" + getNpcId() + "-no_have_memories.htm", false);
						return;
					}
				}
			...
			else if(reply == 3)
			{
				for(int[] item : CHRONICLE_WORD)
				{
					if(!ItemFunctions.haveItem(player, item[0], item[1]))
					{
						showChatWindow(player, "org/l2j/scripts/events/in_the_pursuit_of_letters/" + getNpcId() + "-no_have_chronicle.htm", false);
						return;
					}
				}
		...
		else if(ask == -1001)
		{
			...
			{
				showChatWindow(player, "org/l2j/scripts/events/in_the_pursuit_of_letters/" + getNpcId() + "-no_have_letters.htm", false);
				return;
			}
	...
	{
		return "org/l2j/scripts/events/in_the_pursuit_of_letters/";
	}
}
...
{
    private String[] COMMANDS = new String[] { "cfg" };
    ...
    public boolean useVoicedCommand(String command, Player activeChar, String args)
    {
                ...
                if(param.length == 2)
                {
                    if(param[0].equalsIgnoreCase("lang"))
                    {
                        if(!Config.USE_CLIENT_LANG)
                            ...
                            activeChar.sendMessage(new CustomMessage("org.l2j.gameserver.handler.voicecommands.impl.Cfg.useVoicedCommand.Lang"));
                    }
        ...
        if(!Config.USE_CLIENT_LANG)
        {
            ...
            for(Language lang : getSettings(ServerSettings.class).availableLanguages()) {
                ...
                button = button.replace("<?lang_name?>", Messages.getInstance().getMessage("LangFull", lang));

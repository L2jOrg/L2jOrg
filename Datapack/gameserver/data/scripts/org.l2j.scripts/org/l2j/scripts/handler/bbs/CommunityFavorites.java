	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if("bbsgetfav".equals(cmd))
		{
			...
			try
			{
				...
				String tpl = HtmCache.getInstance().getHtml("scripts/org.l2j.scripts.handler/bbs/bbs_favoritetpl.htm", player);
				while(rset.next())
				{
			...
			String html = HtmCache.getInstance().getHtml("scripts/org.l2j.scripts.handler/bbs/bbs_getfavorite.htm", player);

...
public final class CommunityBoard extends ScriptsCommunityHandler
{
	...
	protected void doBypassCommand(Player player, String bypass)
	{
		...
		if("bbshome".equals(cmd))
		{
			...
			{
				html = HtmCache.getInstance().getHtml("scripts/org.l2j.scripts.handler/bbs/bbs_top.htm", player);
			...
			html = HtmCache.getInstance().getHtml("scripts/org.l2j.scripts.handler/bbs/bbs_homepage.htm", player);
		else if(bypass.startsWith("_bbspage"))
		{
			//Example: "bypass _bbspage:index".
			String[] b = bypass.split(":");
			String page = b[1];
			html = HtmCache.getInstance().getHtml("scripts/org.l2j.scripts.handler/bbs/pages/" + page + ".htm", player);
		}

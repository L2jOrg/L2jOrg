        ...
        for (TeamType team : TeamType.VALUES) {
            ...
            for (DuelSnapshotObject object : list) {
                ...
                if (!checkPlayer(player, false)) {
                    player.sendPacket(new HtmlMessage(0).setFile("org/l2j/scripts/events/custom_event_cancel.htm"));
                    list.remove(object);
                }
            }
        }
    ...
    protected void announceToPlayersWithValidLevel(String str) {
        for (Player player : GameObjectsStorage.getPlayers())
            if (player.isGM() || player.getLevel() >= _minLevel && player.getLevel() <= _maxLevel)
                player.sendPacket(new SayPacket2(0, ChatType.ANNOUNCEMENT, "", String.format(Messages.getInstance().getMessage(player, str), _minLevel, _maxLevel)));
    }

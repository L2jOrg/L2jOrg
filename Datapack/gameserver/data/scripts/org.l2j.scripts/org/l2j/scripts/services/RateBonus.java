...
{
	@Bypass("org.l2j.scripts.services.RateBonus:list")
	public void list(Player player, NpcInstance npc, String[] param)
	{
		...
		String html = HtmCache.getInstance().getHtml("scripts/org.l2j.scripts.services/RateBonus.htm", player);
	...
	@Bypass("org.l2j.scripts.services.RateBonus:get")
	public void get(Player player, NpcInstance npc, String[] param)
	{
		...
		{
			String html = HtmCache.getInstance().getHtml("scripts/org.l2j.scripts.services/RateBonusAlready.htm", player);
		...
		if(success)
		{
			...
			{
				Log.add(player.getName() + "|" + player.getObjectId() + "|rate bonus|" + type + "|" + delay + "|", "org/l2j/scripts/services");
				Functions.show(HtmCache.getInstance().getHtml("scripts/org.l2j.scripts.services/RateBonusGet.htm", player), player);
			}
			else
			{
				if(feeItems.length > 0)

...
public class Roulette
{
	...
	@Bypass("org.l2j.scripts.services.Roulette:dialog")
	public void dialog(Player player, NpcInstance npc, String[] param)
	{
		Functions.show(HtmCache.getInstance().getHtml("scripts/org.l2j.scripts.services/roulette.htm", player).replaceFirst("%min%", Util.formatAdena(Config.SERVICES_ROULETTE_MIN_BET)).replaceFirst("%max%", Util.formatAdena(Config.SERVICES_ROULETTE_MAX_BET)), player);
	}

	@Bypass("org.l2j.scripts.services.Roulette:play")
	public void play(Player player, NpcInstance npc, String[] param)
	{
		...
		String ret = HtmCache.getInstance().getHtml("scripts/org.l2j.scripts.services/rouletteresult.htm", player);

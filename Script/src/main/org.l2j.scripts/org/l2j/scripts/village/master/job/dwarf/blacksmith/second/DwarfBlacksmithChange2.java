 ...
 */
package org.l2j.scripts.village.master.job.dwarf.blacksmith.second;
...
public final class DwarfBlacksmithChange2 extends AbstractNpcAI
{
	...
	public String onAdvEvent(String event, Npc npc, Player player) {
		return switch (event) {
			case "30512-03.htm", "30512-04.htm", "30512-05.htm" -> event;
			case "57" -> classChangeRequested(player, Integer.parseInt(event));
			default -> null;
		};
	}
	...
	private String classChangeRequested(Player player, int classId) {
		...
		if (player.isInCategory(CategoryType.THIRD_CLASS_GROUP)) {
			...
			htmlText = changeClassToWarSmith(player);
		}
		return htmlText;
	}

	private String changeClassToWarSmith(Player player) {
		String htmltext;

...
public final class TakeCastleStart extends AbstractEffect {
    ...
    public void instant(Creature effector, Creature effected, Skill skill, Item item) {
        ...
        var castle = CastleManager.getInstance().getCastle(effected);
        var siegeZone = castle.getSiegeZone();
        if(siegeZone != null && siegeZone.isActive()) {
            siegeZone.broadcastPacketToDefenders(getSystemMessage(SystemMessageId.THE_OPPOSING_CLAN_HAS_STARTED_S1).addSkillName(skill.getId()));
        }
    }

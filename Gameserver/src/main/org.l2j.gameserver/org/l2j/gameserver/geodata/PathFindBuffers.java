...
public class PathFindBuffers
{
    ...
    private static HashIntObjectMap<PathFindBuffer[]> buffers = new HashIntObjectMap<PathFindBuffer[]>();
    private static int[] sizes = new int[0];
    private static Lock lock = new ReentrantLock();

    static
    {
        IntIntMap config = new HashIntIntMap();
        String[] k;
        ...
        for (IntIntPair pair : config.entrySet()) {
            int size = pair.getKey();
    ...
    private static PathFindBuffer create(int mapSize)
    {
        ...
        try
        {
            ...
            if(buff != null)
                buff = org.l2j.commons.lang.ArrayUtils.add(buff, buffer = new PathFindBuffer(mapSize));
            else
            {

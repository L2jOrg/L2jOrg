...
public final class EventParser extends AbstractParser<EventHolder>
{
	...
	public Path getXMLPath() {
		return getSettings(ServerSettings.class).dataPackRootPath().resolve("data/events/");
	}

	@Override
	public Path getCustomXMLPath() {
		return getSettings(ServerSettings.class).dataPackRootPath().resolve("custom/events/");
	}
	...
	protected void readData(Element rootElement) throws Exception
	{
		for(Iterator<Element> iterator = rootElement.elementIterator("event"); iterator.hasNext();)
		{
			...
			try
			{
				eventClass = (Class<Event>) Class.forName("org.l2j.gameserver.model.entity.events.impl." + impl + "Event");
			}

...
import org.l2j.gameserver.mobius.gameserver.FortUpdater;
import org.l2j.gameserver.mobius.gameserver.FortUpdater.UpdaterType;
...
import org.l2j.gameserver.mobius.gameserver.data.xml.impl.StaticObjectData;
...
import org.l2j.gameserver.mobius.gameserver.model.zone.type.L2FortZone;
    ...
    protected void load()
    {
             ...
             PreparedStatement ps = con.prepareStatement("SELECT * FROM fort WHERE id = ?"))
        {
            ...
            if (ownerId > 0)
            {
                ...
                {
                    _FortUpdater[0] = ThreadPoolManager.getInstance().scheduleAtFixedRate(new FortUpdater(this, clan, runCount, UpdaterType.PERIODIC_UPDATE), initial, Config.FS_UPDATE_FRQ * 60000); // Schedule owner tasks to start running
                    if (Config.FS_MAX_OWN_TIME > 0)
                    {
                        _FortUpdater[1] = ThreadPoolManager.getInstance().scheduleAtFixedRate(new FortUpdater(this, clan, runCount, UpdaterType.MAX_OWN_TIME), 3600000, 3600000); // Schedule owner tasks to remove owener
                    }
                }
                else
                {
                    _FortUpdater[1] = ThreadPoolManager.getInstance().schedule(new FortUpdater(this, clan, 0, UpdaterType.MAX_OWN_TIME), 60000); // Schedule owner tasks to remove owner
                }
            }
            else
            {
    ...
    private void updateOwnerInDB()
    {
             ...
             PreparedStatement ps = con.prepareStatement("UPDATE fort SET owner=?,lastOwnedTime=?,state=?,castleId=? WHERE id = ?"))
        {
            ...
            if (clan != null)
            {
                ...
                _FortUpdater[0] = ThreadPoolManager.getInstance().scheduleAtFixedRate(new FortUpdater(this, clan, 0, UpdaterType.PERIODIC_UPDATE), Config.FS_UPDATE_FRQ * 60000, Config.FS_UPDATE_FRQ * 60000); // Schedule owner tasks to start running
                if (Config.FS_MAX_OWN_TIME > 0)
                {
                    _FortUpdater[1] = ThreadPoolManager.getInstance().scheduleAtFixedRate(new FortUpdater(this, clan, 0, UpdaterType.MAX_OWN_TIME), 3600000, 3600000); // Schedule owner tasks to remove owner
                }
            }
            else
            {

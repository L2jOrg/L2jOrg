...
import org.l2j.gameserver.model.actor.recorder.NpcStatsChangeRecorder;
...
import org.l2j.gameserver.network.l2.c2s.RequestItemEnsoul;
import org.l2j.gameserver.network.l2.c2s.RequestTryEnSoulExtraction;
...
import org.l2j.gameserver.templates.npc.Faction;
...
public class NpcInstance extends Creature
{
    ...
    private final IntSet _eventTriggers = new HashIntSet();
    ...
    {
        player.sendMessage(new CustomMessage("org.l2j.gameserver.model.instance.NpcInstance.ErrorBypass").addNumber(getNpcId()).addString(bypass));
    }
    ...
    public void showQuestWindow(Player player)
    {
        ...
        IntObjectMap<QuestInfo> options = new HashIntObjectMap<QuestInfo>();
        ...
        if(options.size() > 1)
        {
            ...
            showQuestWindow(player, options.values().toArray(new QuestInfo[1])[0].getQuest().getId());
        else
            showChatWindow(player, "no-quest.htm", false);
    }

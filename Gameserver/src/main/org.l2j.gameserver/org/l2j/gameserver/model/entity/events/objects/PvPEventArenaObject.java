	...
	public void rewardTeams()
	{
		...
		if(getPointByTeam(teamWin) >= _pvpEvent.getMinKillTeamFromReward())
		{
			...
			for(PvPEventPlayerObject object : teamWin)
			{
				...
				player.sendPacket(new ExShowScreenMessage(new CustomMessage("org.l2j.gameserver.model.entity.events.impl.PvPEvent.win").toString(player), 5000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true, 1, -1, true));
			}
		...
		for(int i = 1; i < _teamsList.size(); ++i)
		{
			...
			{
				Player player = object.getPlayer();
				player.sendPacket(new ExShowScreenMessage(new CustomMessage("org.l2j.gameserver.model.entity.events.impl.PvPEvent.lose").toString(player), 5000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true, 1, -1, true));
			}

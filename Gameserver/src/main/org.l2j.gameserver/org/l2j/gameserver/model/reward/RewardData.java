...
public class RewardData implements Cloneable
{
    ...
    public RewardData(int itemId)
    {
        ...
        if(_item.isArrow() || _item.isBolt() // стрелы не рейтуются
                || (serverSettings.isNoRateEquipment() && _item.isEquipment()) // отключаемая рейтовка эквипа
                || (serverSettings.isNoRateKeyMaterial() && _item.isKeyMatherial()) // отключаемая рейтовка ключевых материалов
                || (serverSettings.isNoRateRecipe() && _item.isRecipe()) // отключаемая рейтовка рецептов
                || ArrayUtils.contains(serverSettings.noRateItems(), itemId)) // индивидаульная отключаемая рейтовка для списка предметов
            _notRate = true;
    }

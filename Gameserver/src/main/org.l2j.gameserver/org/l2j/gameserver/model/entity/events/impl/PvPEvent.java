...
import org.l2j.gameserver.listener.actor.player.impl.EventAnswerListner;
...
import org.l2j.gameserver.model.entity.events.hooks.PvPEventHook;
import org.l2j.gameserver.model.entity.events.objects.PvPEventArenaObject;
import org.l2j.gameserver.model.entity.events.objects.PvPEventPlayerObject;
...
public class PvPEvent extends SingleMatchEvent
{
		...
		switch(name)
		{
			case "registration":
			{
				if(start)
				{
					...
					Announcements.announceToAllFromStringHolder("org.l2j.gameserver.model.entity.events.impl.PvPEvent.registration.start", getEventName());

					CustomMessage askMessage = new CustomMessage("org.l2j.gameserver.model.entity.events.impl.PvPEvent.registration.ask").addCustomMessage(getEventName());
				...
				else
				{
					...
					{
						Announcements.announceToAllFromStringHolder("org.l2j.gameserver.model.entity.events.impl.PvPEvent.registration.cancel", getEventName());
						stopEvent(false);
					}
					else
						Announcements.announceToAllFromStringHolder("org.l2j.gameserver.model.entity.events.impl.PvPEvent.registration.stop", getEventName());
				}
			}
			break;
			case "sort":
	...
	public void reg(Player player)
	{
		if(isRegActive.get() && checkReg(player))
		{
			...
			player.sendMessage(new CustomMessage("org.l2j.gameserver.model.entity.events.impl.PvPEvent.registration.success").addCustomMessage(getEventName()));
		}
	}
	...
	public String getVisibleName(Player player, Player observer)
	{
		if(player != observer)
		{
			if(isBattleActive() && isHideNick())
				return new CustomMessage("org.l2j.gameserver.model.entity.events.impl.PvPEvent.playername").toString(observer);
		}
		return null;
	}
